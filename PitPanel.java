...
/**
 * Defines a panel that represents a pit that holds stones.
...
public class PitPanel extends JPanel implements ChangeListener {
    ...
    /**
     * Constructs a PitPanel that adds a mouse listener, sets style to be used, index of pit
     * to represent, and the datamodel to refer to
    ...
    private class MouseListeners extends MouseAdapter {
        ...
        public void mousePressed(MouseEvent event) {
            ...
            if ((dataModel.isPlayer1Flag() == true) && (dataModel.getStillMyTurn() == true) && (pitIndex >= 8) && (pitIndex <=13) && dataModel.getPits().get(pitIndex) != 0)
            {
                dataModel.updateOldState();
                dataModel.update(pitIndex);
            }
            else if ((dataModel.isPlayer1Flag() == false) && (dataModel.getStillMyTurn() == true) && (pitIndex >= 1) && (pitIndex <=6) && dataModel.getPits().get(pitIndex) != 0)
            {
                dataModel.updateOldState();
                dataModel.update(pitIndex);
            }
